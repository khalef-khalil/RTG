...
export default function ChallengesManager() {
  ...
  const [viewingCompletion, setViewingCompletion] = useState<Challenge | null>(null)
  ...
  const handleViewCompletion = (challenge: Challenge) => {
    setViewingCompletion(challenge)
  }
      ...
      <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-2">
        {sortedChallenges.map(challenge => (
          <ChallengeCard
            ...
            onViewCompletion={() => handleViewCompletion(challenge)}
      ...
      {viewingCompletion && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg max-w-md w-full border border-gray-700">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-white">Completion Details 🎯</h2>
                <button
                  onClick={() => setViewingCompletion(null)}
                  className="text-gray-400 hover:text-white transition-colors"
                >
                  <X size={20} />
                </button>
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-medium text-white mb-2">{viewingCompletion.title}</h3>
                  <p className="text-gray-300 text-sm mb-4">
                    Completed on {new Date(viewingCompletion.completedAt!).toLocaleDateString()}
                  </p>
                </div>

                {viewingCompletion.biggestObstacle && (
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Biggest Obstacle
                    </label>
                    <div className="p-3 bg-gray-700 rounded-lg border border-gray-600">
                      <p className="text-white">{viewingCompletion.biggestObstacle}</p>
                    </div>
                  </div>
                )}

                {viewingCompletion.improvement && (
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      What to Improve
                    </label>
                    <div className="p-3 bg-gray-700 rounded-lg border border-gray-600">
                      <p className="text-white">{viewingCompletion.improvement}</p>
                    </div>
                  </div>
                )}

                {viewingCompletion.pushRating && (
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Push Rating
                    </label>
                    <div className="flex items-center space-x-2">
                        ...
                        {[1, 2, 3, 4, 5].map(rating => (
                          <div
                            ...
                            className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm ${
                              rating <= viewingCompletion.pushRating!
                                ? 'bg-purple-600 border-purple-600 text-white'
                      ...
                      <span className="text-gray-300 text-sm">
                        {viewingCompletion.pushRating}/5
                      </span>
                    </div>
                    <p className="text-xs text-gray-400 mt-1">
                      1 = Easy • 5 = Really pushed my limits
    ...
    <div className={`p-4 rounded-lg border transition-all duration-200 ${
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex space-x-1">
          ...
          {challenge.completed && (challenge.biggestObstacle || challenge.improvement || challenge.pushRating) && (
            <button
              onClick={onViewCompletion}
              className="p-1 hover:bg-gray-700 rounded transition-colors"
              title="View completion details"
            >
              <Target size={14} className="text-blue-400 hover:text-blue-300" />
            </button>
          )}
          <button
            onClick={onStartEdit}
